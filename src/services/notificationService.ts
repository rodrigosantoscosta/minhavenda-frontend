...
const EVENT_MAP: Record<string, (data: SseEventData) => SseNotification> = {
  'pedido.criado': (data) => ({
    type: 'new_order',
    title: 'Pedido criado',
    ...
    for (const line of lines) {
      if (line.startsWith('event:')) {
        ...
        if (currentEventName && currentData) {
          try {
            const parsed = JSON.parse(currentData) as SseEventData
  ...
  try {
    ...
    await readSseStream(response, (eventName, data) => {
      ...
      if (data.pedidoId) {
        ...
        if (newStatus) {
          const known = storageUtil.getItem<Record<string, string>>(KNOWN_STATUSES_KEY, {})
    ...
    logger.warn({ error: (error as Error).message }, 'Erro na conexão SSE — reconectando em 5s')
    scheduleReconnect(addNotification, token)
  }
}
 ...
 */
export function registerOrderStatus(orderId: string, status: string): void {
  const knownStatuses = storageUtil.getItem<Record<string, string>>(KNOWN_STATUSES_KEY, {})
