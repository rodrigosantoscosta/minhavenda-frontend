...
export default function OrderCard({ order, showCancelButton = false, onCancel, cancelling = false }: OrderCardProps): React.JSX.Element {
  const { id, dataCriacao, status, total, valores, itens, endereco } = order

  const orderTotal = total || valores?.total || 0

  const getPrecoValue = (preco: number | { valor?: number } | undefined): number => {
    if (typeof preco === 'object' && preco !== null) return preco.valor || 0
    return preco || 0
  }

  const formatarValor = (valor: number | { valor?: number } | undefined): string => {
