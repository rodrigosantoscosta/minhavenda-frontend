...
const STATUS_COLORS = {
  CRIADO: '#2563EB', PAGO: '#16A34A', ENVIADO: '#D97706', ENTREGUE: '#059669', CANCELADO: '#DC2626',
}

const TOOLTIP_STYLE = { background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 8, fontSize: 12, color: 'hsl(var(--foreground))' }
  ...
  return (
    <AdminCard className="p-6">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider mb-2" style={{ color: 'hsl(var(--muted-foreground))' }}>{label}</p>
...
export default function AdminDashboard() {
  ...
  const kpis = [
    { label: 'Receita Total',      value: stats ? formatBRL(stats.receitaTotal) : '—', icon: FiDollarSign,    color: '#16A34A', bg: 'rgba(22,163,74,0.1)' },
    { label: 'Total de Pedidos',   value: stats ? totalPedidos : '—',                  icon: FiShoppingCart,  color: '#2563EB', bg: 'rgba(37,99,235,0.1)' },
    { label: 'Pedidos Pendentes',  value: stats ? stats.pedidosPorStatus.CRIADO : '—', icon: FiClock,         color: '#D97706', bg: 'rgba(217,119,6,0.1)' },
    { label: 'Alertas de Estoque', value: stats ? stats.estoqueBaixo.length : '—',     icon: FiAlertOctagon,  color: '#DC2626', bg: 'rgba(220,38,38,0.1)' },
  ]
      ...
      <div className="space-y-8">
        ...
        {chartData.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <AdminCard className="p-6">
              <p className="text-sm font-semibold text-foreground mb-5">Pedidos por Status</p>
            ...
            <AdminCard className="p-6">
              <p className="text-sm font-semibold text-foreground mb-5">Distribuição de Status</p>
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={chartData} dataKey="count" nameKey="status" cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3}>
                    {chartData.map(d => <Cell key={d.status} fill={STATUS_COLORS[d.status as keyof typeof STATUS_COLORS] ?? '#6B7280'} />)}
                  </Pie>
                  <Legend formatter={v => <span style={{ color: 'hsl(var(--muted-foreground))', fontSize: 12 }}>{v}</span>} />
        ...
        {stats && stats.estoqueBaixo.length > 0 && (
          <AdminCard>
            <div className="px-6 py-4 flex items-center gap-2" style={{ borderBottom: '1px solid hsl(var(--border))' }}>
              <FiAlertTriangle size={16} style={{ color: '#EF4444' }} />
              <p className="text-sm font-semibold text-foreground">Alertas de Estoque Baixo</p>
                ...
                <tbody>
                  {stats.estoqueBaixo.map((item: { produtoId: string | number; nome: string; quantidade: number }) => (
                    <Tr key={item.produtoId}>
                      <td className="px-6 py-3 text-sm font-sans text-foreground">{item.nome}</td>
        ...
        {degraded && (
          <AdminCard className="p-10 text-center">
            <FiAlertTriangle size={36} className="mx-auto mb-3" style={{ color: 'hsl(var(--border))' }} />
            <p className="text-sm" style={{ color: 'hsl(var(--muted-foreground))' }}>
              Os KPIs e gráficos serão exibidos quando <code className="font-mono text-xs px-1 rounded" style={{ color: 'hsl(var(--muted-foreground))', backgroundColor: 'hsl(var(--muted))' }}>GET /api/admin/dashboard</code> for implementado.
            </p>
          </AdminCard>
        )}
      </div>
