      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <BtnPrimary onClick={handleRequeueAll} disabled={requeueingAll || queues.length === 0}>
            <FiRefreshCw size={16} className={requeueingAll ? 'animate-spin' : ''} />
        ...
        <AdminCard className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              {dica && <p className="text-sm mb-2" style={{ color: '#9CA3AF' }}>{dica}</p>}
        ...
        {queues.length === 0 ? (
          <AdminCard className="p-12 text-center">
            <FiAlertOctagon size={40} className="mx-auto mb-3" style={{ color: '#1E2028' }} />
            <p className="text-sm" style={{ color: '#6B7280' }}>Nenhuma DLQ encontrada</p>
            <p className="text-xs mt-1" style={{ color: '#374151' }}>Todas as mensagens estão sendo processadas normalmente</p>
          </AdminCard>
        ) : (
